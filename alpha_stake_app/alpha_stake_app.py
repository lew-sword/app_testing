...
class SubnetAnalyzer:
    ...
    def get_stake_data(self):
        ...
        for uid in self.get_active_uids():
            ...
            uid_stake_dict[uid] = {"alpha_stake": stake_data_for_uid[1], #gets staked alpha
                                   "stake": stake_data_for_uid[2], #gets total stake like "stake weight" on taostats
    ...
    def to_dataframe(self):
        ...
        df.rename(columns={"index": "UID", "alpha_stake": "Alpha_Stake", "stake": "Stake" , "coldkey": "Coldkey", "v_permit": "V_Permit", "macro_vali": "Macro_Vali" }, inplace=True)
...
def make_chart(df, color_by_v_permit=False, color_by_macro_vali_as=False, color_by_macro_vali_s=False):
    ...
    elif color_by_macro_vali_as:
        # Gold for Macro Validator, Blue for Others on alpha_stake
    ...
    elif color_by_macro_vali_s:
        # Gold for Macro Validator, Blue for Others on stake
        theta_param="Stake"
        color_scheme = alt.Scale(domain=[1, 0], range=["#FFD700", "#4682B4"])
        color_encoding = alt.Color("Macro_Vali:O", scale=color_scheme, title="Macro Validator")
    ...
    return alt.Chart(df).mark_arc().encode(
        ...
        tooltip=["UID", "Stake", "Alpha_Stake", "V_Permit", "Coldkey", "Macro_Vali"]
    ).properties(
...
color_by_macro_vali_as = st.checkbox("Colour by Macro Validator Vs. Others on Alpha_Stake?", value=False)
color_by_macro_vali_s = st.checkbox("Colour by Macro Validator Vs. Others on Stake?", value=False)
...
for inst in [inst_sn1, inst_sn9, inst_sn13, inst_sn25, inst_sn37]:
    ...
    with col2:
        st.altair_chart(make_chart(inst.to_dataframe(), color_by_v_permit, color_by_macro_vali_as,color_by_macro_vali_s), use_container_width=True)  # Show the chart
